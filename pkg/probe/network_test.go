...
func TestNetworkCollector_ExtractProcessName(t *testing.T) {
	// Test extractProcessName method
	collector := NewNetworkCollector(true, 100, true)
	
	// Test with valid process name
	processName := collector.extractProcessName("nginx")
	assert.Equal(t, "nginx", processName)
	
	// Test with process name containing spaces
	processName = collector.extractProcessName("process with spaces")
	assert.Equal(t, "process with spaces", processName)
	
	// Test with empty string
	processName = collector.extractProcessName("")
	assert.Equal(t, "", processName)
	
	// Test with special characters
	processName = collector.extractProcessName("test#00")
	assert.Equal(t, "test#00", processName)
}

func TestNetworkCollector_ExtractListeningPorts(t *testing.T) {
	// Test extractListeningPorts method
	collector := NewNetworkCollector(true, 100, true)
	
	// Test with valid listening ports
	ports := collector.extractListeningPorts("nginx")
	assert.NotNil(t, ports)
	assert.IsType(t, []int{}, ports)
	
	// Test with empty process name
	ports = collector.extractListeningPorts("")
	assert.NotNil(t, ports)
	assert.IsType(t, []int{}, ports)
	
	// Test with non-existent process
	ports = collector.extractListeningPorts("nonexistent-process")
	assert.NotNil(t, ports)
	assert.IsType(t, []int{}, ports)
}
